...
def my_plot(file1, file2, file3, R, X, Y, q, H):
	...
	agent5_line, = plt.plot(agent_optimal, color='r', lw=3, label='agent optimal')
	agent6_line, = plt.plot(aloha_optimal, color='b', lw=3, label='aloha optimal')
	agent7_line, = plt.plot(tdma_optimal, color='g', lw=3, label='tdma optimal')
	# agent8_line, = plt.plot(total_optimal, color='k', lw=3, label='total optimal')
	# plt.grid()
	# plt.title('alpha10')
...
for i in range(1, 2):
	plt.figure(i)
	my_plot('rewards/agent_len1e5_M20_g0.999_q0.5_t2-5_alpha10_r10_%d.txt' % i,
		    'rewards/aloha_len1e5_M20_g0.999_q0.5_t2-5_alpha10_r10_%d.txt' % i,
		     'rewards/tdma_len1e5_M20_g0.999_q0.5_t2-5_alpha10_r10_%d.txt' % i, R=10, X=2, Y=5, q=0.5, H=0.5)
plt.show()