...
def main(history_len, n_actions, ratio, max_iter):
	...
	experience_array = np.zeros((ratio, channel_state_length*2+5))
		...
		if observation != 'U':
			...
			experience_array = np.vstack((experience_array[1:], 
							   np.concatenate([channel_state, 
							   				  [origin_action, duration, agent_reward, aloha_reward, tdma_reward], 
							   				  next_channel_state])))
			if origin_action == 0 and aloha_reward > 1:
				experience_array[-math.ceil(reward):, channel_state_length+3] = aloha_reward / math.ceil(reward)
			if origin_action == 0 and tdma_reward > 1:
				experience_array[-math.ceil(reward):, channel_state_length+4] = tdma_reward / math.ceil(reward)
	...
	with open('rewards/agent_len1e5_M20_g0.999_q0.5_t2-5_alpha50_r10_1.txt', 'w') as my_agent:
		for i in agent_reward_list:
			my_agent.write(str(i) + '   ')
	with open('rewards/aloha_len1e5_M20_g0.999_q0.5_t2-5_alpha50_r10_1.txt', 'w') as my_aloha:
		for i in aloha_reward_list:
			my_aloha.write(str(i) + '   ') 
	with open('rewards/tdma_len1e5_M20_g0.999_q0.5_t2-5_alpha50_r10_1.txt', 'w') as my_tdma:
		for i in tdma_reward_list:
			my_tdma.write(str(i) + '   ') 
	with open('rewards/action_len1e5_M20_g0.999_q0.5_t2-5_alpha50_r10_1.txt', 'w') as my_action:
		for i in action_list:
			my_action.write(str(i) + '   ') 
	with open('rewards/reward_len1e5_M20_g0.999_q0.5_t2-5_alpha50_r10_1.txt', 'w') as my_reward:
		for i in reward_list:
			my_reward.write(str(i) + '   ') 

	### save the number of visits to state
	# with open('states/current_state_len1e5_M20_g0.999_q0.5_t2-5_alpha50_r10_1.txt', 'w') as my_current_state:
	# 	for i in num_current_state:
	# 		my_current_state.write(str(i) + '	')
	# with open('states/total_state_len1e5_M20_g0.999_q0.5_t2-5_alpha50_r10_1.txt', 'w') as my_total_state:
